...
open class RbacAuthorizationHandler(
     ...
     * @return 有效角色与功能权限集合
     */
    open suspend fun getAuthorization(userId: Long, tenantId: Long): UserAuthorization {
     ...
     */
    open suspend fun requirePermissions(userId: Long, tenantId: Long, required: Set<String>, requireAll: Boolean) {
     ...
     * @return 可见菜单树
     */
    open suspend fun getMenus(userId: Long, tenantId: Long): List<MenuNode> {
        val authorization = getAuthorization(userId, tenantId)
     ...
     * @return 授权聚合结果
     */
    private suspend fun loadAuthorization(userId: Long, tenantId: Long): UserAuthorization {
