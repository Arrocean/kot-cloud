...
open class AdminRbacAppService(
    ...
    open suspend fun listRoles(tenantId: Long): List<Role> = queryHandler.listEditableRoles(tenantId)
    /** @param tenantId 租户 ID @return 权限列表 */
    open suspend fun listPermissions(tenantId: Long): List<Permission> = queryHandler.listPermissions(tenantId)
    /** @param tenantId 租户 ID @return 菜单配置列表 */
    open suspend fun listMenus(tenantId: Long): List<Menu> = queryHandler.listMenus(tenantId)
    /** @param userId 用户 ID @param tenantId 租户 ID @return 用户角色 ID 集合 */
    open suspend fun getUserRoleIds(userId: Long, tenantId: Long): Set<Long> = queryHandler.getUserRoleIds(userId, tenantId)

    /** @param command 创建角色命令 @param tenantId 租户 ID @return 已创建角色 */
    open suspend fun createRole(command: CreateRoleCommand, tenantId: Long): Role = commandHandler.createRole(
    ...
    open suspend fun updateRole(id: Long, command: UpdateRoleCommand, tenantId: Long): Role {
    ...
    open suspend fun deleteRole(id: Long, tenantId: Long) = commandHandler.deleteRole(id, tenantId)
    /** @param id 角色 ID @param permissionIds 权限 ID 集合 @param tenantId 租户 ID */
    open suspend fun assignRolePermissions(id: Long, permissionIds: Set<Long>, tenantId: Long) =
    ...
    open suspend fun assignUserRoles(userId: Long, roleIds: Set<Long>, tenantId: Long) =
        commandHandler.assignUserRoles(userId, roleIds, tenantId)

    /** @param command 创建菜单命令 @param tenantId 租户 ID @return 已创建菜单 */
    open suspend fun createMenu(command: CreateMenuCommand, tenantId: Long): Menu = commandHandler.createMenu(command.toDraft(tenantId))

    /** @param id 菜单 ID @param command 更新菜单命令 @param tenantId 租户 ID @return 更新后菜单 */
    open suspend fun updateMenu(id: Long, command: UpdateMenuCommand, tenantId: Long): Menu {
    ...
    open suspend fun deleteMenu(id: Long, tenantId: Long) = commandHandler.deleteMenu(id, tenantId)
}
