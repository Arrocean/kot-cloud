...
open class RbacCommandHandler(
     ...
     */
    @Transactional
    open suspend fun assignDefaultRole(userId: Long, tenantId: Long) {
     ...
     */
    @Transactional
    open suspend fun assignRolePermissions(roleId: Long, permissionIds: Set<Long>, tenantId: Long) {
     ...
     */
    @Transactional
    open suspend fun createRole(draft: RoleDraft): Role {
     ...
     */
    @Transactional
    open suspend fun updateRole(role: Role): Role {
     ...
     */
    @Transactional
    open suspend fun deleteRole(roleId: Long, tenantId: Long) {
        val role = requireEditableRole(roleId, tenantId)
        if (role.builtIn) {
            throw ServiceExceptionFactory.exception(GlobalErrorCodeConstants.FORBIDDEN)
        }
     ...
     */
    @Transactional
    open suspend fun createMenu(draft: MenuDraft): Menu {
        validateMenu(draft)
     ...
     */
    @Transactional
    open suspend fun updateMenu(menu: Menu): Menu {
        validateMenu(
            MenuDraft(
                code = menu.code,
     ...
     */
    @Transactional
    open suspend fun deleteMenu(menuId: Long, tenantId: Long) {
     ...
     * @return 可编辑角色
     */
    private suspend fun requireEditableRole(roleId: Long, tenantId: Long): Role {
     ...
     * @param draft 待校验菜单草稿
     */
    private suspend fun validateMenu(draft: MenuDraft) {
