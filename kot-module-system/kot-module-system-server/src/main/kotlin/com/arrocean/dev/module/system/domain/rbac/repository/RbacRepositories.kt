...
interface RoleRepository {
    ...
    suspend fun findByCode(code: String, tenantId: Long): Role?
    /** @param ids 角色 ID 集合 @return 已存在角色列表 */
    suspend fun findByIds(ids: Collection<Long>): List<Role>
    /** @param tenantId 租户 ID @return 租户内角色列表 */
    suspend fun findAll(tenantId: Long): List<Role>
    /** @param draft 角色创建草稿 @return 已保存角色 */
    suspend fun save(draft: RoleDraft): Role
...
interface PermissionRepository {
    ...
    suspend fun findAll(tenantId: Long): List<Permission>
    /** @param ids 权限 ID 集合 @return 已存在权限列表 */
    suspend fun findByIds(ids: Collection<Long>): List<Permission>
    /** @param userId 用户 ID @param tenantId 租户 ID @return 用户经由角色获得的有效权限 */
    suspend fun findEffectiveByUserId(userId: Long, tenantId: Long): List<Permission>
}
...
interface MenuRepository {
    ...
    suspend fun findAll(tenantId: Long): List<Menu>
    /** @param code 菜单编码 @param tenantId 租户 ID @return 菜单；不存在时为空 */
    suspend fun findByCode(code: String, tenantId: Long): Menu?
    /** @param draft 菜单创建草稿 @return 已保存菜单 */
    suspend fun save(draft: MenuDraft): Menu
...
interface UserRoleRepository {
    ...
    suspend fun findRoleIdsByUserId(userId: Long, tenantId: Long): Set<Long>
    /** @param roleId 角色 ID @param tenantId 租户 ID @return 拥有角色的用户 ID 集合 */
    suspend fun findUserIdsByRoleId(roleId: Long, tenantId: Long): Set<Long>
    /** @param userId 用户 ID @param roleIds 角色 ID 集合 @param tenantId 租户 ID */
    suspend fun replaceRoles(userId: Long, roleIds: Set<Long>, tenantId: Long)
    /** @param userId 用户 ID @param roleId 角色 ID @param tenantId 租户 ID */
    suspend fun addRole(userId: Long, roleId: Long, tenantId: Long)
}
...
interface RolePermissionRepository {
    ...
    suspend fun replacePermissions(roleId: Long, permissionIds: Set<Long>, tenantId: Long)
}
