...
open class RbacQueryHandler(
     ...
     */
    open fun listEditableRoles(tenantId: Long): List<Role> = roleRepository.findAll(tenantId)
     ...
     */
    open fun getRole(id: Long, tenantId: Long): Role? = roleRepository.findById(id)
     ...
     * @return 权限列表
     */
    open fun listPermissions(tenantId: Long): List<Permission> = permissionRepository.findAll(tenantId)
     ...
     * @return 菜单列表
     */
    open fun listMenus(tenantId: Long): List<Menu> = menuRepository.findAll(tenantId)
     ...
     * @return 菜单；不存在时为空
     */
    open fun getMenu(id: Long, tenantId: Long): Menu? = menuRepository.findAll(tenantId).firstOrNull { it.id == id }
     ...
     * @return 角色 ID 集合
     */
    open fun getUserRoleIds(userId: Long, tenantId: Long): Set<Long> =
